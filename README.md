...
构建门禁三级路由：CI → 专用构建机 → 本机兜底，额度耗尽自动降级；**本机兜底通过 ≠ 结案，只是欠账**——销账内嵌在下一次构建启动时自动补验（不依赖 cron）。**任何门禁默认 10 分钟硬上限**（超时强杀留证，修构建不许调上限），npm/pip 国内源运行时注入 + 命名缓存卷（实测热缓存耗时减半）。专治 CI 额度静默失效、本机性能不足、构建机连通不稳、Docker Hub 拉不动。
