...
## 降级为什么会「装了却没生效」（2026-08 实战复盘）

一次 CI 因**账单失败**停摆的真实事故：三级降级一条都没自动生效，人只能手工
build/rsync/deploy——正是这套模板要根除的东西。根因全部是「规定与工具互相打架」，
每条都值得在自己的部署里对照检查：

1. **门禁必须认 git worktree。** `[ -d "$REPO/.git" ]` 判不出 worktree
   （worktree 的 `.git` 是**文件**，内容是指向主检出的 gitdir）。而多 agent 并行的
   交接协议恰恰要求用 worktree 隔离写入——于是规定的流程被规定的门禁拒之门外，
   人只能绕过门禁。**判据一律 `git -C "$REPO" rev-parse --git-dir`**，别猜文件类型。
   （本模板已修，见 `build-gate.sh` / `reverify.sh`。）

2. **门禁镜像必须与生产镜像同源。** 事故中生产跑 `python:3.13`，而构建机的通用
   builder 是 `node22 + python3.14`——**用 3.14 验、生产跑 3.13，验的不是要发的东西**，
   绿了也没有意义。`BUILDER_IMAGE` 支持按仓库覆盖，**新增仓库时必须设到与其
   Dockerfile 同源的镜像**，不要用「手边那个通用 builder」。

3. **验证命令必须能在干净环境跑，且与 CI 同款。** 事故中该仓库的验证命令写成
   `.venv/bin/python3 -m pytest`——只在开发者本机成立，一进干净容器就 `exit 127`。
   正确写法是照抄 CI 的安装+运行序列（含 dev 依赖），两边同源才不会漂移。
   **「只在我机器上能跑的命令」不是验证命令**，它让降级路径在最需要的时候失效。

4. **三级降级只覆盖 CI（验证），不覆盖 CD（构建产物 + 推仓库 + 部署）。**
   2026-08-17 起，这个缺口按失效模式拆成两半：
   - **托管额度/账单故障（最常见）——已关闭。** self-hosted job 不受额度/账单影响
     （当日实证：hosted 3 秒被拒、self-hosted 照常调度，GHCR/API/git 全正常），
     CI 与 CD 的全部 job 走仓库变量路由切自建 runner 即可继续发布，见 ADR 0006 与
     `templates/ci/runner-canary.yml`。切换是变量操作不是代码改动。
   - **托管方整体不可用（git/API/Packages 全断）——仍未覆盖。** 该模式下必须**事前**
     写明「无法按政策发布」，不要临场手搓 build/rsync 补救。

### 顺带：self-hosted runner 不计费

多数托管 CI（GitHub Actions 等）**不对 self-hosted runner 的分钟数计费**。
把 PR 门禁放到自有构建机上，额度耗尽与账单失败都影响不到它——
靠「每次手动降级」是治标，把门禁迁到自有机器才是治本。
前提是那台机器能连到 CI 控制面，见下面的连通性判据。

2026-08-17 账单欠费日实证：hosted job 被 3 秒拒（`payments have failed`，`runner_name` 空、0 步），
同账号同仓的 self-hosted job 照常调度跑完；GHCR 推拉、API、git 均正常。
落地形态：`runs-on` 不写死，由仓库变量路由（ADR 0006），故障日先跑
`templates/ci/runner-canary.yml` 实证再切——**网络与调度结论都有时效性，只认当日 canary**。

### 弱网诊断判据（先分域名测，别笼统说「没网」）

同一次事故里，构建机被误判成「整机失联」，实际上：

| 目标 | 结果 | 含义 |
|---|---|---|
| `github.com` / `api.github.com` | **000，0.02s** | 立刻被 RST —— **被阻断** |
| 容器仓库 / 镜像站 / 包源 | 200–302 | 通 |

**判据：`000` 且 `<0.05s` = 被阻断；`000` 且接近 timeout = 真不通。**
DNS 也要单独验（解析对不对 ≠ 连得上）。粒度不到这一层，就会把
「能构建、能推镜像、只是当不了 runner」误判成整机故障。

**overlay 网络（Tailscale 等）走中继时 RTT 可达 1–2 秒**，`ConnectTimeout 8`
必然超时——同一次事故据此把一台 up 23 天、load 0.4 的健康机器判成挂了。
诊断顺序：先看链路是 direct 还是 relay，再用长超时复核，**然后**才允许下
「机器挂了」的结论。
