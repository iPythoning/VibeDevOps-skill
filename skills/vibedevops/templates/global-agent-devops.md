...
- **新增仓库一律走这条线**：CI/CD 从 VibeDevOps 模板起步（pr-check/deploy 已内置变量路由），在统一构建机注册本仓 runner——注册即自动进入账单故障 failover 纳管（自动发现，无需登记）。不走模板的仓库必须写明原因。每台构建机上一个 runner 只许一个监管者（系统级 service 单元），叠加监管会抢会话。
