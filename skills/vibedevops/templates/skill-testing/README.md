...
## negative case 不打分，只判通过与否

**rubric 衡量的是「这条轨迹展现了多少该 skill 的流程」，而 negative case 的正确
行为恰恰是「不展现」**——用同一把尺子量它，行为完全正确的负例也会得低分。

实测：一个改错别字的负例，agent 加载 skill 后依据「什么时候不适用」正确排除、
全程只用一条命令链完成任务——四个维度里三个是 0（没做反向自证、没穷举解释、
没找第二通道，因为**都不需要**），加权 0.4 判 fail。两个评分模型还因此分歧 1.1
（1.5 vs 0.4）——**框架自己的分歧检测正确地指出了这里有问题**。

所以 negative case 走二元判定：`final: null`，verdict 只看「有没有套用不该用的
流程」。它也因此天然不参与退化比对（没有分数可比）。
...
补跑单个 case 用 `only`——一轮完整评测是 20+ 个 agent，为一个失败 case 全量重跑是纯浪费：

```
Workflow({ scriptPath: "…/run-skill-eval.js", args: {
  skill: "evidence-discipline",
  fixtures: "…/fixtures.yaml",
  only: "neg-1-typo"          // 逗号分隔可多个
}})
```

**补跑要开新 run，别用 resume**：resume 按 `(prompt, opts)` 命中缓存，而 fixture 是
外部文件——改了 fixture 但 load 的 prompt 没变时，它会命中缓存返回**旧 fixture 的内容**，
而且没有任何提示。这本身就是一种观测陷阱。
